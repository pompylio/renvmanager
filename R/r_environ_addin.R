...
renviron_addin <- function() {
      ...
      miniUI::miniTabPanel(
        ...
        miniUI::miniContentPanel(
          shiny::radioButtons(
            inputId = "env_target",
            label = "Salvar em:",
            choices = c("Projeto" = "project", "Usuário" = "user", "Outro" = "custom"),
            inline = TRUE),
  ...
  server <- function(input, output, session) {
    ...
    get_file_path <- shiny::reactive({
      switch(
        input$env_target,
        "project" = file.path(rstudioapi::getActiveProject() %||% getwd(), ".Renviron"),
    ...
    output$select_item_ui <- shiny::renderUI({
      ...
      if (file.exists(path)) {
        ...
        choices <- c("Nova Variável" = "NEW", vars)
        shiny::selectInput("existing_var", "Editar existente:", choices = choices)
      }
    })

    # Preenche os inputs quando uma variável é selecionada
    ...
    shiny::observeEvent(input$done, {
      ...
      invisible(
        shiny::stopApp())
      if (rstudioapi::hasFun("restartSession")) rstudioapi::restartSession()
    })

    shiny::observeEvent(input$cancel, { shiny::stopApp() })
  }

  shiny::runGadget(ui, server, viewer = shiny::dialogViewer("Gerenciar .Renviron", width = 500, height = 500))
}
